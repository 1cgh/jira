...
Функция Поиск(СтрокаПоиска, НачинаяС = Неопределено, МаксКоличествоРезультатов = Неопределено,
	...
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(СтруктураЗапроса, НачинаяС, "startAt");
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(СтруктураЗапроса, МаксКоличествоРезультатов, "maxResults");
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(СтруктураЗапроса, ПроверятьЗапрос, "validateQuery");
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(СтруктураЗапроса, МассивПолей, "fields");
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(СтруктураЗапроса, РасширитьПо, "expand");
...
Функция СоздатьЗадачу(ДанныеЗадачи, ОбновлятьИсторию = Неопределено) Экспорт
	...
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(ПараметрыЗапроса, ОбновлятьИсторию, "updateHistory");
...
Функция ПолучитьЗадачу(ИдентификаторКлючЗадачи, Поля = Неопределено, РасширитьПо = Неопределено,
	...
	ПараметрыЗапроса = Новый Структура;
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(ПараметрыЗапроса, Поля, "fields");
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(ПараметрыЗапроса, РасширитьПо, "expand");
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(ПараметрыЗапроса, Свойства, "properties");
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(ПараметрыЗапроса, ОбновлятьИсторию, "updateHistory");
...
// TODO: вынести в отдельный класс для доски.
// Спринты
//
// Параметры:
//   ИдентификаторДоски - Число - идентификатор доски, к которой относится спринт.
//   НачинаяС - Число, Неопределено - индекс, начиная с которого выдаются результаты. По умолчанию 0.
//   МаксКоличествоРезультатов - Число, Неопределено - макс. количество задач в результате поиска. По умолчанию 50.
//   Состояние - Строка - список состояний через запятую для отбора спринтов.
//
//  Возвращаемое значение:
//   Массив - массив объектов типа Спринт.
//
Функция Спринты(ИдентификаторДоски, НачинаяС = Неопределено, МаксКоличествоРезультатов = Неопределено,
	...
	СлужебныеМетоды.ВставитьЗначениеВСтруктуруЕслиОпределено(ПараметрыЗапроса, Состояние, "state");

	ДополнительныеПараметры = Новый Структура("ПараметрыЗапроса", ПараметрыЗапроса);

	АдресРесурса = СтрШаблон("/rest/agile/latest/board/%1/sprint", ИдентификаторДоски);

	РезультатЗапроса = РезультатЗапроса(АдресРесурса, "GET", ДополнительныеПараметры);

	Спринты = РезультатЗапроса.JSON();

	Результат = Новый Массив;
	Для каждого ДанныеСпринта Из Спринты["values"] Цикл
		Спринт = Новый Спринт(ЭтотОбъект, ДанныеСпринта);
		Результат.Добавить(Спринт);
	КонецЦикла;
	Возврат Результат;

КонецФункции

Функция РезультатЗапроса(АдресРесурса, HTTPМетод, ДополнительныеПараметры = Неопределено) Экспорт
